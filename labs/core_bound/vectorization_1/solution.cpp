...
using simd_t = std::array<int16_t, sequence_count_v>;
using simd_sequence_t = std::array<simd_t, sequence_size_v>;

simd_sequence_t Transport(std::vector<sequence_t> const &sequences)
{
  simd_sequence_t simd_sequence;
  for (size_t row = 0; row < sequences.size(); ++row) {
    sequence_t sequence = sequences[row];
    for (size_t col = 0; col < sequence.size(); ++col) {
      simd_sequence[col][row] = sequence[col];
    }
  }
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
 ...
 /*   sequence_t const &sequence1 = sequences1[sequence_idx];
    ...
    simd_sequence_t simd_sequences1 = Transport(sequences1);
    simd_sequence_t simd_sequences2 = Transport(sequences2);
    ...
    for (unsigned col = 1; col <= simd_sequences2.size(); ++col) {
      ...
      for (unsigned row = 1; row <= simd_sequences1.size(); ++row) {
        ...
        for (size_t k = 0; k < sequence_count_v; ++k)
        {
          best_cell_score[k] = last_diagonal_score[k] + (simd_sequences1[row - 1][k] == simd_sequences2[col - 1][k] ? match[k] : mismatch[k]);
        }
        for (size_t k = 0; k < sequence_count_v; ++k)
        {
